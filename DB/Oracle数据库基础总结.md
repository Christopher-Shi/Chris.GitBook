...
block是pl/sql的基本逻辑单元，分为声明部分，执行部分，异常处理部分三部分，且可以嵌套使用，基本的使用示例如下：
...
Oracle中的变量只能在pl/sql代码块中使用。


1.标量变量： 即只能存放单个数值的变量，Oracle中使用最多的变量类型，其定义和使用方法如下：  	
...
2.复合变量：即可以存放多个数值的变量，它包括记录类型，PL/SQL表（类似于数组），varray类型；

示例：
	set SERVEROUTPUT ON;  
	declare type jilu_type is RECORD (v_ename varchar2(5),v_tt int );  
	jilu jilu_type ;  
	begin  
    select name,id into jilu from t1;  
    dbms_output.put_line(jilu.v_ename);  
	end;  

3.参照变量：是指用于存放数值指针的变量。通过使用参照变量，可以使得应用程序共享相同对象，从而降低占用空间。分为ref cursor和ref obj_type两种


### Oracle中的临时表TEMPORARY  ###

使用：对于庞大的数据我们只要查询其中一小部分结果集这样我们可以借助临时表。临时表不存在并发行为，因为他们对于当前会话都是独立的。（它默认是事务级别的）
Oracle临时表分为会话级临时表和事务级临时表，它们的定义如下：

    create global temporary table aaa(id number) on commit oreserve rows;
    insert into aaa values(100);   
    当你在打开另一个会话窗口的时候再次查询，表内的数据就查询不到了。
...
- inner join： 两张表能够互相匹配到的数据才显示。  
- full outer join：两张表所有的数据都会显示，不存在任意一方为null。 
...
expdp/impdp适用于中小型数据库系统,适用于oracle11g之后，之前使用emp/imp，两者的区别是前者属于服务端功能，后者属于客户端功能，它们的备份信息不能共享。

备份整库：`expdp system/gaoryorc dumpfile =full.dmp full=y logfile=full.log `

恢复整库：`Impdp system/gaoryorc DIRECTORY=dpdump DUMPFILE=full.dmp`
